...
public class MarkDown {
    ...
    public static void setMarkDownText(String sourceStr, TextView targetTv, int[] olNumber, Map<String, Integer> olNumberMap) {
        ...
        if (!TextUtils.isEmpty(sourceStr)) {
            ...
            if (MarkDownPatterns.H1.matcher(sourceStr).matches()) {
                setHSpannableString(MarkDownStatements.H1, sourceStr, spannableStringBuilder);
                olNumber[0] = 1;
            } else if (MarkDownPatterns.H2.matcher(sourceStr).matches()) {
                setHSpannableString(MarkDownStatements.H2, sourceStr, spannableStringBuilder);
                olNumber[0] = 1;
            } else if (MarkDownPatterns.H3.matcher(sourceStr).matches()) {
                setHSpannableString(MarkDownStatements.H3, sourceStr, spannableStringBuilder);
                olNumber[0] = 1;
            } else if (MarkDownPatterns.OL.matcher(sourceStr).matches()) {
                ...
                //有序列表索引自增
                olNumber[0]++;
            } else if (MarkDownPatterns.UL.matcher(sourceStr).matches()) {
    ...
    private static void setItalicSpannableString(SpannableStringBuilder spannableStringBuilder) {
        ...
        Matcher matcher = MarkDownPatterns.ITALIC.matcher(sourceStr);
        while (matcher.find()) {
    ...
    private static void setBoldSpannableString(SpannableStringBuilder spannableStringBuilder) {
        //解析所有粗体文字
        String sourceStr = spannableStringBuilder.toString();
        Matcher matcher = MarkDownPatterns.BOLD.matcher(sourceStr);
        while (matcher.find()) {
    ...
    private static void setHyperLinkSpannableString(SpannableStringBuilder spannableStringBuilder) {
        //解析所有超链接
        String sourceStr = spannableStringBuilder.toString();
        Matcher matcher = MarkDownPatterns.HYPER_LINK.matcher(sourceStr);
        while (matcher.find()) {
            ...
            //找出Url并设置可点击
            String url = "";
            Matcher urlMatcher = MarkDownPatterns.URL.matcher(hyperLinkStr);
            if (urlMatcher.find()) {
    ...
    private static void setImageSpannableString(final SpannableStringBuilder spannableStringBuilder, final TextView targetTv) {
        // 解析所有图片
        String sourceStr = spannableStringBuilder.toString();
        Matcher matcher = MarkDownPatterns.IMAGE.matcher(sourceStr);
        while (matcher.find()) {
            String hyperLinkStr = matcher.group();
            int start = sourceStr.indexOf(hyperLinkStr);
            int end = start + hyperLinkStr.length();
